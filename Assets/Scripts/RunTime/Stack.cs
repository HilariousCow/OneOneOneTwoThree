 ...
 /*   private int _flipBothHash = Animator.StringToHash("FlipBoth");
    ...
    void LateUpdate()
    {
        if (!_isPreview)
        {
            ...
            float dotRight = Vector3.Dot(Camera.main.transform.right, Vector3.down);
            dotRight = Mathf.Abs(dotRight);//1 == portrait
            dotDown = (1f - Mathf.Clamp01(dotDown));


            Vector3 targetPosition = Vector3.Lerp(transform.parent.position, transform.parent.TransformPoint(Vector3.up * 10f), dotDown * (1f - dotRight));
            transform.localScale = Vector3.one*(1f + (dotDown * (1f - dotRight))  * 3f);
